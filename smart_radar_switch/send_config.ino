void send_config(){
 ...
 // html += "<input type=\"checkbox\" id=\"switchjson\" "+json_act()+" ><text> Использовать формат JSON </text>";
  ...
  html += "<input type=\"text\" id=\"topic\" name=\"topicname\" placeholder=\"Пример: [имя_топика]/temp\"  value=\""+String(settings.mqtt_topic)+"\" required><br>";
...
String full_cong(){
    ...
    data += "<text> Идентификатор устройства: <b>"+ch_id+"</b></text><br>";
    data += "<text> Имя корня топика: <b>"+String(settings.mqtt_topic)+"</b></text><br>";
